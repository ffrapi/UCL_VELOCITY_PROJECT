...
INPUT=$SHAREDFOLDER/$SPECIES/02a_mapped_MODE
OUTPUT=$SHAREDFOLDER/$SPECIES/02a_mapped_MODE
TAIL="RG.bam"

#Set up ARRAY job
#ls $INPUT/*RG.bam | awk -F "/" '{print $NF}' | awk -F "." '{print $1}' >> mode.names3
NAME=$(sed "${SGE_TASK_ID}q;d" mode.names3)
...
CREATE_INDEX=true" >> 02b.1_MarkDup.mode.log
